...
def record_spikes(N_Excit=384, N_Inhib=96, weight_scaling_factor=5.33,
                                t_stimulus_start=100 * b2.ms, t_stimulus_duration=9999 * b2.ms, coherence_level=0.,
                                stimulus_update_interval=30 * b2.ms, mu0_mean_stimulus_Hz=250.,
                                stimulus_std_Hz=20.,
                                N_extern=1000, firing_rate_extern=9.8 * b2.Hz,
                                w_pos=1.90, f_Subpop_size=0.25,  # .15 in publication [1]
                                max_sim_time=1000. * b2.ms, stop_condition_rate=None,
                                monitored_subset_size=512, E_leak_excit = -70.0 * b2.mV):
    """
    ...
    def update_poisson_stimulus(t):
        if t >= t_stimulus_start and t < t_stimulus_end:
            ...
            stim_info_a.append(rate_A) ## added by MSY 
            stim_info_b.append(rate_B) ## added by MSY
            # print("stim on. rate_A= {}, rate_B = {}".format(rate_A, rate_B))
        else:
            # print("stim off")
            poissonStimulus2A.rates = 0.
    ...
    def get_spikes(pop, monitored_subset_size):  ## added by MSY
            """
            Internal helper.
            Args:
                pop:
            ...
            spike_monitor = SpikeMonitor(pop, record=idx_monitored_neurons)
            #voltage_monitor = StateMonitor(pop, "v", record=idx_monitored_neurons)
            return monitored_subset_size, idx_monitored_neurons, spike_monitor
    ...
    subset_size, neuron_index, spike_monitor = get_spikes(excit_pop_A, monitored_subset_size)
    ...
    ret_vals['test_spikes_subset_size'] = subset_size
    ret_vals['test_spikes_neuron_index'] = neuron_index
    ret_vals['test_spikes_spikes'] = spike_monitor

    ret_vals['N_Group_A'] = N_Group_A  ## number of neurons that are included in population A 
